...
void LifeGUI::resize(int numRows, int numColumns) {
    ...
    int currX=max(0, upperLeftX-1);
    int currY=max(0, upperLeftY - 1);
    int maxX=numColumns * cellDiameter + 2;
    int maxY=numRows * cellDiameter + 2;
    window.drawRect(currX, currY, maxX, maxY);
    /*
    for(int i=1; i<numColumns; i++){
        window.drawLine(currX+i*cellDiameter, currY, currX+i*cellDiameter, maxY);
    }
    */
}
...
void LifeGUI::drawCell(int row, int column, bool alive) {
    ...
    if(age == 0) {
        window.fillRect(upperLeftX + column * cellDiameter, upperLeftY + row * cellDiameter,
                        cellDiameter - 1, cellDiameter - 1);
    } else {
        window.fillOval(upperLeftX + column * cellDiameter, upperLeftY + row * cellDiameter,
                        cellDiameter - 1, cellDiameter - 1);
    }
    ages[row][column] = age;
}
